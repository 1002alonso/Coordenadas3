(function() {
  ...
  */
  ...
  function addStyleBlock() {
    if ( d3.select('.datamaps-style-block').empty() ) {
      ...
      .html('.datamap path.datamaps-graticule { fill: none; stroke: #fff; stroke-width: 0.5px; stroke-opacity: .5; pointer-events: none; } .datamap .labels {pointer-events: none;} .datamap path {stroke: #FFFFFF; stroke-width: 1px;} .datamaps-legend dt, .datamaps-legend dd { float: left; margin: 0 3px 0 0;} .datamaps-legend dd {width: 20px; margin-right: 6px; border-radius: 3px;} .datamaps-legend {padding-bottom: 20px; z-index: 1001; position: absolute; left: 4px; font-size: 12px; font-family: "Helvetica Neue", Helvetica, Arial, sans-serif;} .datamaps-hoverover {display: none; font-family: "Helvetica Neue", Helvetica, Arial, sans-serif; } .hoverinfo {padding: 4px; border-radius: 1px; background-color: #FFF; box-shadow: 1px 1px 5px #CCC; font-size: 12px; border: 1px solid #CCC; } .hoverinfo hr {border:1px dotted #CCC; }');
    }
  }
 ...
 function handlePoligono(layer,data,options){
     var self=this,
         svg =this.swg
        
        console.log("probandoPoligo");
             if ( !data || (data && !data.slice) ) {
      throw "Datamaps Error - polig must be an array";
    }

      for (var i=0; i < data.length; i++){
        data[i]=defaults(data[i],data[i].optinos);
      ...
      polydata = [{"x":0.0, "y":25.0},
        {"x":8.5,"y":23.4},
        {"x":13.0,"y":21.0},
        {"x":19.0,"y":15.5}];

      var poly = layer.selectAll('path.datamaps-arc').data( polydata, JSON.stringify );
        ...
        .attr("points",function(d) { 
         return d.map(function(d) {
            ...
            latLng = self.latLngToXY(d.latitude, d.longitude);
             ...
             alert(latLng[0]);
            return [scaleX(latLng[0]),scaleY(latLng[1])].join(",");
             }).join(" ");
               })
        .attr("stroke","black")
        .attr("stroke-width",2);
 ...
 function handleLabelsEdit ( layer, options ) {
...
sasx=$(".datamap").attr("width")/2;


// sasy=$(".datamap").attr("height")/2;
...
// alert(sasy);
 ...
 function handleLabelsIcono ( layer, options ) {
    var self = this;
    options = options || {};
    var labelStartCoodinates = this.projection([-67.707617, 42.722131]);
        ...
        layer.append("text")
          ...
          .style("font-size", (options.fontSize || 10) + 'px')
          .style("font-family", options.fontFamily || "map-icons")
          .style("fill", options.labelColor || "#000")
          .text( options.text );
        return "bar";
  ...
  ***************************************/
  ...
  function Datamap( options ) {
    ...
    this.addPlugin('labels2', handleLabelsEdit);
    this.addPlugin('labelsIco', handleLabelsIcono);
    this.addPlugin('graticule', addGraticule);
    this.addPlugin('poligono', handlePoligono);
